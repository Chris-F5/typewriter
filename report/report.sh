...
markup_text="(markup_text.py -w $body_width -s 10 -S 10 -a j -A l ; echo 'glue 10')"
...
unlisted youtube video that demonstrated all objectives.
The video shows the compilation of 3 documents, one for each objective
category.
In each document, text is labled with the objective number that is
demonstrated.
...
*1. Parsing Input* the escape sequences for different types of text have been
successful.
When writing my report, I found it intuitive to surround text with starts (*)
to make it bold or underscores (_) to make it italic.
Headers and footnotes were also used effectively in my report.

*2. Break Text Into Lines* The line breaking algorithm has produced visually
appealing paragraphs for my report. The insertion of spaces between words has
been essential. Although hyphenation was not used in the report, it is a
supported feature albeit one which is not so useful for single column A4
pages.
Multiple font sizes in the same paragraph are supported but also not used in
this report.

*3. Break lines into pages.* Lines have been fitted into pages in
this report.
In some rare occasions, orphan and widow lines generated by 'line_break'
reduce the ascetics of a page, but this is a low-priority issue that does not
