    ...
    int max_width, int max_height, struct stack *graphics_stack)
{
  ...
  graphic.first
    = stack_allocate(graphics_stack, sizeof(struct pdf_content_instruction));
    ...
    struct stack *graphics_stack)
{
  ...
  if (element->type >= sizeof(layout_rules) / sizeof(layout_rules[0])) {
